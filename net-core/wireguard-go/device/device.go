...
type Device struct {
	// awg — параметры обфускации AmneziaWG; nil для обычного WireGuard.
	awg *AWGConfig
...
func NewDevice(ctx context.Context, tunDevice tun.Device, bind conn.Bind, logger *Logger, workers int) *Device {
	device := new(Device)
	// Параметры обфускации AmneziaWG. nil означает обычный WireGuard.
	// Читаются из контекста тем же способом, что и остальные зависимости.
	device.awg = AWGConfigFromContext(ctx)
	device.awg.logSetup()
